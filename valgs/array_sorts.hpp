#ifndef ARRAY_SORTS_HPP
#define ARRAY_SORTS_HPP
...
template <typename T>
void down_heap(T *array, size_t count, size_t root_index, std::function<int(T, T)> compare)
{
    size_t left_index = root_index * 2 + 1;
    size_t right_index = root_index * 2 + 2;
    size_t largest_index = root_index;

    if (left_index < count && compare(array[largest_index], array[left_index]) < 0)
    {
        largest_index = left_index;
    }

    if (right_index < count && compare(array[largest_index], array[right_index]) < 0)
    {
        largest_index = right_index;
    }
    if (largest_index != root_index)
    {
        std::swap(array[largest_index], array[root_index]);
        down_heap(array, count, largest_index,compare);
    }
}

template <typename T>
void heap_sort(T *array, size_t count, std::function<int(T, T)> compare)
{

    for (signed_size_t root_index = count / 2 - 1; root_index >= 0; root_index--)
    {
        down_heap(array, count, (size_t)root_index, compare);
    }
    for (signed_size_t current = count - 1; current > 0; current--)
    {
        std::swap(array[0], array[current]);
        down_heap(array, (size_t)current, (size_t)0, compare);
    }
}

#endif //ARRAY_SORTS_HPP